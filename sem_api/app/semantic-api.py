...
def get_config():
    ...
    config = [
        ...
        {"type": "calc", "api": "age", "name": "Age in Years", "label": "Age"},
    ]
...
def get_all_data(patient_ids: str = None, downloadFile: str = None):
    ...
    if downloadFile is None:
        ...
        for row in results["data"]:
            bigstr += ",".join(row) + "\n"
        headers = {
            "Content-Disposition": "attachment; filename={}.csv".format(downloadFile)
        }
...
def get_age_data(min: int = None, max: int = None):
    ...
    result_set = defaultdict(list)
    for row in results:
        if (min == None or int(row["age"]) >= min) and (
            max == None or int(row["age"]) <= max
        ):
